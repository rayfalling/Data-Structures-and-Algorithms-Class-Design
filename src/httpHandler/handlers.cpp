...
namespace http_handlers{
	bool handlers::login(std::string, std::string body, std::string method, mg_connection* c,
	                     OnRspCallback rsp_callback) {
		transform(method.begin(), method.end(), method.begin(), tolower);
		if (method != "post")return false;
		const auto json_object = jsonHelper<data_access_layer::login>::parse_json_string(std::move(body));

		auto search_results = users[json_object.email];
		...
		const json response = {{"status", !login_failed}};

		rsp_callback(c, response.dump());
		return true;
	}

	bool handlers::registry(std::string, std::string body, std::string method, mg_connection* c,
	                        OnRspCallback rsp_callback) {
		transform(method.begin(), method.end(), method.begin(), tolower);
		if (method != "post")return false;
		const auto json_object = jsonHelper<data_access_layer::registry>::parse_json_string(std::move(body));
		...
		const json response = { {"status", create_verified} };
	...
	void handlers::registry_router(server::http_server* server) {
		server->add_handler("/api/login", login);
		server->add_handler("/api/registry", registry);
	}
