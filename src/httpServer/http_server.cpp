...
std::unordered_map<std::string, req_handler> server::http_server::s_handler_map;
std::unordered_set<mg_connection *> server::http_server::s_websocket_session_set_;
...
void server::http_server::add_handler(const std::string& url, req_handler req_handler) {
...
void server::http_server::handle_http_event(mg_connection* connection,
	...
	const auto url = std::string(http_req->uri.p, http_req->uri.len);
	const auto body = std::string(http_req->body.p, http_req->body.len);
	const auto method = std::string(http_req->method.p, http_req->method.len);
	const auto it = s_handler_map.find(url);
	if (it != s_handler_map.end()) {
		const auto handle_func = it->second;
...
int server::http_server::is_websocket(const mg_connection* connection) {
	return MG_F_IS_WEBSOCKET & connection->flags;
}
...
void server::http_server::handle_websocket_message(mg_connection* connection,
	...
	if (event_type == MG_EV_WEBSOCKET_HANDSHAKE_DONE) {
		...
		// 添加 session
		s_websocket_session_set_.insert(connection);
		...
		if (is_websocket(connection)) {
			...
			// 移除session
			if (s_websocket_session_set_.find(connection) !=
				s_websocket_session_set_.end())
				s_websocket_session_set_.erase(connection);
		}
	}
}
...
void server::http_server::broadcast_websocket_msg(std::string msg) {
	for (mg_connection* connection : s_websocket_session_set_)
		mg_send_websocket_frame(connection, WEBSOCKET_OP_TEXT, msg.c_str(),
		                        strlen(msg.c_str()));
}
