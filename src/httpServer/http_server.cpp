...
std::string									HttpServer::s_web_dir = "./webroot";
...
bool HttpServer::Start() {
	...
	mg_connection *connection =
		mg_bind(&m_mgr, m_port.c_str(), HttpServer::OnHttpWebsocketEvent);
...
void HttpServer::HandleHttpEvent(mg_connection *connection,
	...
	if (it != s_handler_map.end()) {
		...
		handle_func(url, body, method, connection, &HttpServer::SendHttpRsp);
	}
